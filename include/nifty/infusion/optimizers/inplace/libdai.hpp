...
///\cond
namespace detail_nifty_infusion_dai{
...
struct BpParameters{
    ...
    std::string to_dai_param_string()const{
        ...
        else if(update_rule==SEQUENTIAL_FIXED){
            ur = "SEQFIX";
        }
        else if(update_rule==SEQUENTIAL_RANDOM){
            ur = "SEQMAX";
        }
        else if(update_rule==SEQUENTIAL_MAX){
            ur = "SEQRND";
        }
        ss <<"BP["
           <<"updates="<<ur<<","
    ...
    UpdateRule update_rule{PARALLEL};
    std::size_t verbose;
    std::size_t logDomain_;
...
using LibdaiJunctionTree = Libdai<VARIABLE_SPACE, detail_nifty_infusion_dai::JunctionTreeParameters>;

template<class VARIABLE_SPACE>
using LibdaiBp = Libdai<VARIABLE_SPACE, detail_nifty_infusion_dai::BpParameters>;
