...
TEST_CASE("test_random_chain") {
    ...
    typedef UnaryFunction<2>   UnaryFunctionType;
    typedef PottsFunction<2>   PottsFunctionType;
    typedef XTensorFunction<2> XTensorFunctionType;
    ...
    for(auto vi=0; vi<n_variables-1; ++vi){
        ...
        XTensorFunctionType rand_function =  xt::random::randn<float>({2,2});

        ad3_opt.add_factor({vi, vi+1}, rand_function);
        qpbo_opt.add_factor({vi, vi+1}, rand_function);
        libdai_junction_tree_opt.add_factor({vi, vi+1}, rand_function);
    }
